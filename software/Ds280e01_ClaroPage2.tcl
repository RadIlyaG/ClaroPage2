...
# ***************************************************************************
# WritePage0_CLEI
# set gaSet(cleiNum) [CLEI_to_Hex  INMJ600DRA] ; EntryBootMenu ; WritePage0_CLEI
# ***************************************************************************
...
# WritePage0_SN_CLEI
# ***************************************************************************
proc WritePage0_SN_CLEI {} {
  global gaGui buffer buff  gaSet
  set com $gaSet(comDut)
  puts "[MyTime] WritePage0_SN_CLEI"; update
  ...
  Send $com "d2 00,03,11,02\r" "\[boot" 2
  set ret [regexp {([\w\.]{32})\s} $buffer ma var1]
  if {$ret!=1} {
    set gaSet(fail) "Page3 read fail." ; update
    return -1	  
  }
  set gaSet(barcode) ""
  set var1 [string trim [regsub -all -- {\.} $var1 " "]]
  foreach hex $var1 {
    set asci [format %c [scan $hex %x]]
    append gaSet(barcode) $asci
  }
  puts "$gaSet(barcode)"
  ...
  if {$gaSet(serNum)==""} {
    set lSerNum [lrange $resL 0 7]
  } else {
    set lSerNum [list 00 00 00 [string range $gaSet(serNum) 0 1] [string range $gaSet(serNum) 2 3]\
     [string range $gaSet(serNum) 4 5] [string range $gaSet(serNum) 6 7]  [string range $gaSet(serNum) 8 9]] 
  }
  
  set gaSet(cleiNum) [CLEI_to_Hex $gaSet(cleiCode)]
  set lClei [list [string range $gaSet(cleiNum) 0 1]   [string range $gaSet(cleiNum) 2 3]\
               [string range $gaSet(cleiNum) 4 5]   [string range $gaSet(cleiNum) 6 7]\
               [string range $gaSet(cleiNum) 8 9]   [string range $gaSet(cleiNum) 10 11]\
               [string range $gaSet(cleiNum) 12 13] [string range $gaSet(cleiNum) 14 15]\
               [string range $gaSet(cleiNum) 16 17] [string range $gaSet(cleiNum) 18 19]] 
  ...
  set l2 [lrange $resL 18 end]   
  set page0 [concat $lSerNum $lClei $l2]      		
...
proc WritePage2 {} {
  ...
  set offSet 00
  # Read Page 2
  Send $com "d2 $device,02,32,$offSet\r" "\[boot" 2
  set ret [regexp {([\w\.]{47})\s+([\w\.]{47})} $buffer var var1 var2]
  if {$ret!=1} {
    set gaSet(fail) "Page2 check fail." ; update
    return -1	  
  }
  set var1 [string trim [regsub -all -- {\.} $var1 " "]]
  set var2 [string trim [regsub -all -- {\.} $var2 " "]]
  ...
  set var2 [string trim [regsub -all -- {\.} $var2 " "]]
  set res "$var1 $var2"
  if {[string match *$page2* $res]==0} {
    set gaSet(fail) "Page2 result fail." ; update
    puts "res:$res"
    puts "pag:$page2"
    #puts stderr "Page$page result fail." 
    return -1    
